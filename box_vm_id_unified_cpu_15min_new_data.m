...
% Clear data
all_cpu_pct = []; 
all_box_cpu_pct = []; all_vm_cpu_pct = [];
all_box_cpu_pct_great_than_zero_index = [];
all_vm_cpu_pct_great_than_zero_index = [];
...
for box_id = 1 : numel(box)
    ...
    box_vm_time_series{box_id}{1} = [box_vm_time_series{box_id}{1}, box{box_id}(:,5:6)];
...
for vm_id = 1 : numel(vm)
    ...
    while time_idx <= time_len
        if box_id ~= temp_vm(time_idx, 3)
            ...
            summary_of_vm_diff_box{vm_id}{box_no}(:,4:5) = temp_vm(start_idx:time_idx-1,6:7);
...
for box_id = 1 : numel(box_vm_time_series_summary)
    ...
    sum_box_cpu_pct = box_vm_time_series_summary{box_id}{1,1}(row_idx,4)/100;
    X = X(row_idx, :);
    time = time(row_idx, :);
    ...
    box_all_ratio_temp = sum_box_cpu_pct(sum_box_cpu_pct >= 0);
...
set(gcf, 'paperpositionmode', 'auto');
print('-depsc2','-r300', strcat(path,'VM_CPU_WEIGHTED_FITTING_R^2'));
...
set(gcf, 'paperpositionmode', 'auto');
print('-depsc2','-r300', strcat(path,'VM_CPU_WEIGHTED_BOX_RATIO'));
...
set(gcf, 'paperpositionmode', 'auto');
print('-depsc2','-r300', strcat(path,'VM_CPU_WEIGHTED_VM_RATIO'));
...
% set(gca, 'xscale', 'log');
ylabel('CDF', 'fontsize', 18); 
xlabel('BOX CPU USED PCT(%)', 'fontsize', 18);
set(gcf, 'paperpositionmode', 'auto');
print('-depsc2','-r300', strcat(path,'VM_CPU_WEIGHTED_BOX_ALL_RATIO'));
