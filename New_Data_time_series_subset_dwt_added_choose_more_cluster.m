...
use_silluette_or_not = true;

need_to_load_cluster_result = true;

if need_to_load_cluster_result
    if use_silluette_or_not
        load (strcat(path, 'dtw_cidx_upper_bound'))
    else
        load (strcat(path, 'dtw_cidx_determined_cluster'))
    end
end
...
%     
    ...
    if ~ need_to_load_cluster_result
        ...
        if use_silluette_or_not
            ...
            cluster_num = no_cluster(box_id);      
        end
        cidx = cluster(z, 'maxclust', cluster_num);
        cluster_dtw{box_id} = cidx;
    else
        cidx = cluster_dtw{box_id};
    end
...
%         cand_id = cand_id - 1;
%     end
    final_representative_cluster = representative_cluster;
    time_len = numel(all_time_series(:,1));
    ...
    sample = {}; counts = {};
    for cand_id = 1 : numel(final_representative_cluster)
        X_fit = X;
        % if the time series is in the representative
        represent_id = final_representative_cluster(cand_id);
%         if numel(find(final_representative_cluster == represent_id)) == 1
        vm_no = represent_id;
...
%             y_fit = max(0, Y - r);
            ...
            box_vm_time_series_demands = all_time_series_used_cap(:, represent_id) / ticket_thres * 100;
            sample{vm_no} = unique(box_vm_time_series_demands);
            if sample{vm_no}(1) ~= 0
                sample{vm_no} = [0; sample{vm_no}];
            end
            ...
            % allocation will not furthur degrade the performance, especially
            % for the *truncated* case.
            lower_bound = max(all_time_series_used_cap(:, represent_id));
            [~, idx_lower] = min(sample{vm_no} >= lower_bound);
            ...
            r = Y - all_time_series(:, represent_id);
            abs_error(end+1) = mean(abs(r));
            ape(end+1) = nanmean(abs(r) ./ Y);
            r_square(end+1) = 0; %stats(1); 
        end
...
if ~need_to_load_cluster_result
    if use_silluette_or_not
        save(strcat(path, 'dtw_cidx_upper_bound'), 'cluster_dtw');
    else
        save(strcat(path, 'dtw_cidx_determined_cluster'), 'cluster_dtw');   
    end
end
...
if ~use_silluette_or_not
    print('-depsc2','-r300', strcat(path,'determined_cluster_box_vm_ape_error_cdf'));
else
    print('-depsc2','-r300', strcat(path,'upper_bound_box_vm_ape_error_cdf'));
end
...
set(gcf, 'paperpositionmode', 'auto');
if ~use_silluette_or_not
    print('-depsc2','-r300', strcat(path,'determined_cluster_vm_reduction_cdf_same_cluster_no_xcf'));
else
    print('-depsc2','-r300', strcat(path,'upper_bound_vm_reduction_cdf_same_cluster_no_xcf'));
end
...
ylabel('CDF');
title(strcat('DTW Cluster, mean = ', {' '}, mat2str(int32(nanmean(REDUCED_PCT*100))), '%'));
set(gca,'xlim', [-100 100]); set(gca, 'xtick', [-100 :20 : 100]);
set(gca,'ylim', [0 1]); set(gca, 'ytick', [0 : 0.1 : 1]);
set(gca, 'fontsize',15);
set(gcf, 'paperpositionmode', 'auto');
if ~use_silluette_or_not 
    print('-depsc2','-r300', strcat(path,'determined_cluster_ticket_reduction_cdf_same_cluster_no_xcf'));
else
    print('-depsc2','-r300', strcat(path,'upper_bound_ticket_reduction_cdf_same_cluster_no_xcf'));
end
