...
mkdir('../New_Data_time_series_linear_subset_vif_cpu_mem_figure');
path = '../New_Data_time_series_linear_subset_vif_cpu_mem_figure/';
...
%     
    ...
    for vm_id = 2 : size_box
        ...
        if numel(true_counts_mem{vm_id-1}) ~= 0
            original_ticket_mem = original_ticket_mem + true_counts_mem{vm_id-1}(idx);  
        end
    end
...
% cdf: APE: BOX-CPU,MEM; VM-CPU,MEM
fig = figure;
set(fig,'Position',[200, 200, 600, 400]);
set(gca, 'fontsize', 18);
box_mem_test = ALL_APE_MEM(1, ~isinf(ALL_APE_MEM(1,:)));
vm_mem_test = ALL_APE_MEM(2, ~isinf(ALL_APE_MEM(2,:)));

[x_bc, f_bc] = ecdf(ALL_APE(1,:)*100);
[x_bm, f_bm] = ecdf(box_mem_test*100);
[x_vc, f_vc] = ecdf(ALL_APE(2,:)*100);
[x_vm, f_vm] = ecdf(vm_mem_test*100);
plot(f_bc, x_bc, 'k-', 'linewidth', 2)
hold on
plot(f_bm, x_bm, 'r--', 'linewidth', 2)
h = legend('BOX CPU', 'BOX RAM');
...
xlabel('BOX: Aboslute Percentage Error of CPU/RAM USED PCT (%)', 'fontsize', 18);
...
set(fig,'Position',[200, 200, 600, 400]);
set(gca, 'fontsize', 18);
plot(f_vc, x_vc, 'k-', 'linewidth', 2)
hold on
plot(f_vm, x_vm, 'r--', 'linewidth', 2)
h = legend('VM CPU', 'VM RAM');
set(h, 'box','on','location','southeast','fontsize',18);
...
% title('Fitting Error');
ylabel('CDF', 'fontsize', 18); 
xlabel('VM: Aboslute Percentage Error of CPU/RAM USED PCT (%)', 'fontsize', 18);
set(gcf, 'paperpositionmode', 'auto');
...
set(gca,'xlim', [0 100]); set(gca, 'xtick', [0 : 10 : 100], 'fontsize', 18);
set(gca, 'ylim', [0 1]); set(gca, 'ytick', [0 : 0.1 : 1], 'fontsize', 18);
% set(gca, 'xscale', 'log');
h = legend('Alg-2');
set(h, 'box','on','location','southeast','fontsize',18);
ylabel('CDF', 'fontsize', 15); 
xlabel('Reduced Percentage of Used Predictors (%)', 'fontsize', 15);
set(gcf, 'paperpositionmode', 'auto');
print('-depsc2','-r300', strcat(path,'cdf_reduce_vm_percent'));

% vm reduction CDF, big box
big_box_idx = ORIGINAL_VM_NUM >= 40; 
vm_reduced_pct = VM_REDUCED_PCT(big_box_idx) * 100;
fig = figure;
set(fig,'Position',[200, 200, 600, 400]);
set(gca, 'fontsize', 18);
[f_reduce, x_reduce] = ecdf(vm_reduced_pct);
...
% ticket reduction CDF
fig = figure;
set(fig,'Position',[200, 200, 600, 400]);
set(gca, 'fontsize', 18);
[f_reduce, x_reduce] = ecdf(REDUCED_PCT*100);
[f_reduce_prio, x_reduce_prio] = ecdf(PRIO_REDUCED_PCT*100);
plot(x_reduce_prio, 1-f_reduce_prio, 'k-', 'linewidth', 2);
hold on
plot(x_reduce, 1-f_reduce, 'r--', 'linewidth', 2);
set(gca,'xlim', [0 100]); set(gca, 'xtick', [0 : 10 : 100], 'fontsize', 18);
set(gca, 'ylim', [0 1]); set(gca, 'ytick', [0 : 0.1 : 1], 'fontsize', 18);
% set(gca, 'xscale', 'log');
h = legend('Alg-1', 'Alg-2');
set(h, 'box','on','location','southeast','fontsize',18);
ylabel('CCDF', 'fontsize', 15); 
xlabel('Reduced Percentage of Tickets (%)', 'fontsize', 15);
set(gcf, 'paperpositionmode', 'auto');
print('-depsc2','-r300', strcat(path,'cdf_reduce_ticket_percent_cpu'));

% ticket reduction CDF
fig = figure;
set(fig,'Position',[200, 200, 600, 400]);
set(gca, 'fontsize', 18);
[f_reduce, x_reduce] = ecdf(REDUCED_PCT_MEM*100);
[f_reduce_prio, x_reduce_prio] = ecdf(PRIO_REDUCED_PCT_MEM*100);
plot(x_reduce_prio, 1- f_reduce_prio, 'k-', 'linewidth', 2);
hold on
plot(x_reduce, 1-f_reduce, 'r--', 'linewidth', 2);
set(gca,'xlim', [0 100]); set(gca, 'xtick', [0 : 10 : 100], 'fontsize', 18);
...
xlabel('Reduced Percentage of Tickets (%)', 'fontsize', 15);
set(gcf, 'paperpositionmode', 'auto');
print('-depsc2','-r300', strcat(path,'cdf_reduce_ticket_percent_mem'));
