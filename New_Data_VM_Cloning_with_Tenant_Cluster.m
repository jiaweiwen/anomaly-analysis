...
load (strcat(moment_path, 'MOMENT_STD_COEFF_CLUSTER_', mat2str(moments(usage_tail_idx))));
load (strcat(moment_path, 'MOMENT_MEAN_COEFF_CLUSTER_', mat2str(moments(usage_tail_idx))));
load (strcat(moment_path, 'cluster_results_', mat2str(moments(usage_tail_idx))));

fig_path = strcat('../New_Data_box_moments_fit/', mat2str(ticket_thres),'_Target_Cluster_Added_Fig/');
mkdir(fig_path);
...
num_cluster = numel(MOMENT_MEAN_COEFF_CLUSTER(:,1));
CONF_UPPER = []; 
for cluster_id = 1 : num_cluster
    for mean_id = 1 : numel(mean_usage)
        used_mean = mean_usage(mean_id);

        p1 = MOMENT_MEAN_COEFF_CLUSTER(cluster_id, :);
        usage_mean = used_mean * p1(1) + p1(2);
        
        p2 = MOMENT_STD_COEFF_CLUSTER(cluster_id, :);
        usage_std = used_mean * used_mean * p2(1) + used_mean * p2(2) + p2(3);
       
        CONF_UPPER(cluster_id, mean_id) = max(min(100, usage_mean + 2*usage_std), 0);
    end
end
...
mean_usage_thres = [];
for cluster_id = 1 : num_cluster
    idx = max(find(CONF_UPPER(cluster_id, :) <= ticket_thres));
    mean_usage_thres(cluster_id) = mean_usage(idx);
end
...
marker = {'*', 'o', '^'};
for cluster_id = 1 : num_cluster
    plot(mean_usage, CONF_UPPER(cluster_id, :), 'color', cc{cluster_id}, ...
         'linestyle', line_style{cluster_id}, 'marker', marker{cluster_id}, ...
         'linewidth', 1.5);
    hold on
end
...
plot(get(gca,'xlim'), [ticket_thres ticket_thres], 'b-', 'linewidth', 1.5);
h = legend('Class1: Tail Upper Bound', 'Class2: Tail Upper Bound', ...
           'Class3: Tail Upper Bound', 'Usage Threshold');
...
for tenant_id = 1 : numel(unique_tenant)
    ...
    % determine which usage threshold this tenant should use
    cand_idx = find(cluster_results(:, 1) == tenant_id);
    cluster_id = cluster_results(cand_idx, 2);
    usage_thres_target = mean_usage_thres(cluster_id);
    ...
    for box_id = 1 : num_box
        ...
        if sum(great_than_thres)/numel(great_than_thres) >= 1 - moments(usage_tail_idx)/100
            ...
            if mean_box_usage > usage_thres_target
                problem_box(end+1, 1:2) = [box_id, (mean_box_usage - usage_thres_target) / 100 * mean_box_cap];
            else
...
%             vm_ticket_rank(idx) = 1 : num_vm-1;
            ...
            if mean_box_usage >= usage_thres_target
                ...
                safe_box_for_clone(end+1, :) = [box_id, (usage_thres_target - mean_box_usage) / 100 * mean_box_cap, rest_mem_cap];   
            end
        end
    ...
    USED_TENANT_CLUSTER(end+1, :) = [tenant_id, cluster_id];
...
test_usage = [usage_size, mean_usage_thres, ticket_thres - usage_size];% 0 : usage_size : ticket_thres - usage_size;
sig_level = 0.05;
for test_id = 1 : numel(test_usage)
    ...
    for moment_id = usage_tail_idx : usage_tail_idx
        ...
        ylabel('CDF');
%         if test_id == 2
%             title1 = strcat('Mean Box CPU Usage: ', {' '}, mat2str(usage_thres_target));
%         else
...
%                       'p_{test1:test2} =', {' '}, mat2str(round(p3, 4)));
        ...
        if test_id >= 2
            ...
            hold on
            plot([ticket_thres ticket_thres], get(gca,'ylim'), 'm-', 'linewidth', 1)
